...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default='/data/EfficientVRNet')
    parser.add_argument('--wandb_path', type=str, default='/data/EfficientVRNet/wandb', help='path of saving wandb files locally')
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train=model_train, model=model, ema=ema, yolo_loss=yolo_loss, loss_history=loss_history,
                          loss_history_seg=loss_history_seg, eval_callback=eval_callback, 
                          optimizer=optimizer,epoch=epoch, epoch_step=epoch_step, epoch_step_val=epoch_step_val, gen=gen, gen_val=gen_val, Epoch=UnFreeze_Epoch,
                          cuda=Cuda, fp16=fp16, scaler=scaler, weight_save_dir=weight_save_dir, dice_loss=dice_loss,
                          focal_loss=focal_loss, cls_weights=cls_weights, num_class_seg=num_classes_seg,
                          local_rank=local_rank)
